...
How **fpv-strike-drone** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `fpv-strike-drone` lives in the **Drone / counter-UAS** cluster.
...
| from | relation | to |
|---|---|---|
| `fpv-strike-drone` | JSON findings compose with | [`uaslog`](https://github.com/cognis-digital/uaslog), [`awesome-drone-warfare-osint`](https://github.com/cognis-digital/awesome-drone-warfare-osint), [`frontline-drones`](https://github.com/cognis-digital/frontline-drones) |
| `fpv-strike-drone` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `fpv-strike-drone` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `fpv-strike-drone` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the drone / counter-uas cluster.** `fpv-strike-drone` output feeds the next tool:
```bash
fpv-strike-drone ... --format json > out.json          # this tool's findings (see `fpv-strike-drone --help`)
uaslog ... < out.json                      # the cluster sibling consumes them
```
...
fpv-strike-drone ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
fpv-strike-drone ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
fpv-strike-drone ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `fpv-strike-drone`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `fpv-strike-drone` slots
into the **Counter-UAS maritime** stack below.
