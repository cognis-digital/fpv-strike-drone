...
**fpv-strike-drone** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `fpv-strike-drone` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
fpv-strike-drone ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
fpv-strike-drone ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
fpv-strike-drone ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
```
...
works whatever `fpv-strike-drone` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="fpv-strike-drone") for rec in records]   # records = your JSON output
